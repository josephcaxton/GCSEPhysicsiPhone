...
- (void)viewDidAppear:(BOOL)animated {
	...
	if ([self isDataSourceAvailable] == YES) {
		...
		if ([CollectionofArrays count] > 0) {
			...
			//Due to screen limitation we need to produce graph max 14 results
			if ([CollectionofArrays count] > 14) {
				
				int removeAmount = [CollectionofArrays count] - 14;
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation duration:(NSTimeInterval)duration{
	...
	if (interfaceOrientation == UIInterfaceOrientationPortrait ) {
		...
		ThisTable.frame = CGRectMake(0, 0, SCREEN_HEIGHT + 30 , SCREEN_WIDTH);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.row == 0){
		if ([CollectionofArrays count] > 0){
			...
			NSString *FilePath = [[NSBundle mainBundle] pathForResource:@"Graph_Iphone" ofType:@"png"];
