...
@synthesize CollectionofArrays,FinalString,graph,MaxValueForYValue,Totals,data,sets,dates,DataDump;
...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *ClearLogs = [[UIBarButtonItem alloc] initWithTitle:@"Clear" style:UIBarButtonItemStylePlain target:self action:@selector(ClearAllLogs:)];
...
-(void)configurePlots {
   ...
   CPTMutableLineStyle *majorGridLineStyle = [CPTMutableLineStyle lineStyle];
    ...
    //Plot
    BOOL firstPlot = YES;
    for (NSString *set1 in [[sets allKeys] sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)]) {
        CPTBarPlot *plot        = [CPTBarPlot tubularBarPlotWithColor:[CPTColor blueColor] horizontalBars:NO];
        plot.lineStyle          = barLineStyle;
        CGColorRef color        = ((UIColor *)[sets objectForKey:set1]).CGColor;
    ...
    graph.legendDisplacement  = CGPointMake(180.0, -10.0);
